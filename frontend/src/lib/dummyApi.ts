import { ApiNodeData } from "@/services/nodeService"; // 타입을 가져와서 명확하게 지정합니다.
...
export const createInitialDummyNodes = (topic: string): ApiNodeData[] => {
  ...
  // [수정] 반환되는 배열의 타입을 명확히 하여 타입 에러를 해결합니다.
  return [
...
export const createSubDummyNodes = (parentNodeName: string, parentId: number): ApiNodeData[] => {
  console.log(`[DUMMY API] 하위 노드 생성: 부모 - ${parentNodeName}`);
  const newNodes: ApiNodeData[] = [
    { id: nextNodeId++, name: `${parentNodeName}의 예시`, type: 'CHILD', parentId },
    { id: nextNodeId++, name: `${parentNodeName}의 중요성`, type: 'CHILD', parentId },
    { id: nextNodeId++, name: `${parentNodeName} 관련 메모`, type: 'TXT', parentId },
  ];
  return newNodes;
};
